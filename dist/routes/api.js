...
router.post('/projects', async (req, res) => {
    try {
        const { name, slug, clientId, clientName, clientPhone, category, location, description, budget, startDate, deadline, beforeImage, afterImage, wipImages, images, featured, } = req.body;
...
// ==========================================
router.get('/design-requests', async (_req, res) => {
    try {
...
// ==========================================
router.get('/tree-doctor', async (_req, res) => {
    try {
...
router.post('/tree-doctor', async (req, res) => {
    try {
...
router.get('/plant-health', async (req, res) => {
    try {
...
router.post('/quotations', async (req, res) => {
    try {
        ...
        const subtotal = items.reduce((acc, item) => acc + (parseFloat(item.amount) || (parseFloat(item.unitPrice) * parseFloat(item.quantity))), 0);
...
router.get('/accounting', async (_req, res) => {
    try {
...
router.get('/admin/overview', async (_req, res) => {
    try {
        const [customersCount, runningProjectsCount, completedProjectsCount, employeesCount, maintenanceCount, treeDoctorRequestsCount, pendingQuotationsCount,] = await Promise.all([
